...
backup()
{
	...
	#Config and Service
	tar -cvf "$BACKUP_DIR/.cfg" /etc >cfg_bk 2>&1
	echo "\nBACKUP: Config Done\n"
	tar -cvf "$BACKUP_DIR/.www" /var/www >www_bk 2>&1
	echo "\nBACKUP: Web Done\n"
	tar -cvf "$BACKUP_DIR/.bin" /bin /sbin /usr/bin /usr/sbin >bin_bk 2>&1
	echo "\nBACKUP: Bin Done\n"
...
mv $(which curl) $(which curl)20 
mv $(which wget) $(which wget)20
echo "Done"
